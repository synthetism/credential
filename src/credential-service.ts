...
function base64urlEncode(data: string): string {
  try {
    ...
    if (nodeBuffer && typeof nodeBuffer === 'object' && 'from' in nodeBuffer) {
      return (nodeBuffer as { from: (data: string) => { toString: (encoding: string) => string } }).from(data).toString('base64url');
    }
    ...
    if (browserBtoa && typeof browserBtoa === 'function') {
      return (browserBtoa as (data: string) => string)(data).replace(/\+/g, '-').replace(/\//g, '_').replace(/=/g, '');
    }
...
function base64urlDecode(data: string): string {
  try {
    ...
    const nodeBuffer = (globalThis as Record<string, unknown>)?.Buffer;
    if (nodeBuffer && typeof nodeBuffer === 'object' && 'from' in nodeBuffer) {
      return (nodeBuffer as { from: (data: string, encoding: string) => { toString: () => string } }).from(data, 'base64url').toString();
    }
...
export class CredentialService {
  ...
  static async createJWTProof(
    ...
    try {
      ...
      return Result.fail(`Failed to create JWT proof: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  static async verifyJWTProof(
    ...
    try {
      ...
      try {
        ...
        // Verify signature
        const isValidSignature = await keyManager.verify(signingInput, signature, publicKey);
        ...
        return Result.fail(`Failed to parse JWT payload: ${parseError instanceof Error ? parseError.message : String(parseError)}`);
      }
    } catch (error) {
      return Result.fail(`Failed to verify JWT proof: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async issueVC<S extends BaseCredentialSubject>(
    ...
    try {
      ...
      return Result.fail(`Failed to issue credential: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async verifyVC(
    ...
    try {
      ...
      if (proof.type === 'JwtProof2020' && proof.jwt) {
        return await CredentialService.verifyJWTProof(credential, this.keyManager, options);
      }
      
      return Result.fail(`Unsupported proof type: ${proof.type}`);
    } catch (error) {
      return Result.fail(`Failed to verify credential: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
   ...
   */
  static validateCredential(credential: unknown): Result<W3CVerifiableCredential> {
    try {
      ...
      const requiredFields = ['@context', 'id', 'type', 'issuer', 'issuanceDate', 'credentialSubject', 'proof'];
      ...
      return Result.fail(`Invalid credential structure: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
}
...
export const CredentialUtils = {
  ...
  createSimpleKeyManager(): KeyManager {
    ...
    return {
      ...
      async createKey(type = 'Ed25519'): Promise<{ keyId: string; publicKey: string }> {
